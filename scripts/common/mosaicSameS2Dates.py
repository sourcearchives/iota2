...
import random,os,gdal,osr,shutil,argparse
...
def launchFit(noDataM,noDataR,tileFolder,currentTile,sensorName,S2Folder,S2Bands,masks):
	...
	try:workingDirectory = os.environ["TMPDIR"]
	except : workingDirectory = None
	...
	for currentSameDate in sameDateFolder:
		...
		for currentBand in S2Bands:
			...
			for i in range(len(tmpList)):
				if i != rasterPosDate : tmp.append(tmpList[i])
				else : tmp.append(currentDate)
			outName = "_".join(tmp)
			#############################
