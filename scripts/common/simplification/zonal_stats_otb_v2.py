...
def zonal_stats_otb(vecteur, r_validity, r_classif, r_classifier, field, outwd, version, out, ndept, maskmer="", split=""):
    ...
    shutil.copyfile(poly_class_stats, os.path.join(out, os.path.basename(poly_class_stats)))
    
    # Mask for sample selection
    if maskmer is not None:
        print "mask"
        tifMasqueMer = osof.otb_bandmath_ram([r_classif], "im1b1*0", 2, 8, False, True, outwd + "/masque_mer.tif")
        maskmerbuff = os.path.join(outwd, os.path.splitext(maskmer)[0] + 'buff.shp')
        BufferOgr.bufferPoly(maskmer, maskmerbuff, 500)
        command = "gdal_rasterize -burn 1 %s %s"%(maskmerbuff, tifMasqueMer)
        os.system(command)
        mask = osof.otb_bandmath_ram([r_classif, tifMasqueMer], \
                                     "((im1b1==0) || (im1b1==51)) && (im2b1==0)?0:1", \
                                     2, 8, False, True, outwd + "/mask.tif")
    else:
        mask = ""
    ...
    shutil.copyfile(sample_selection, os.path.join(out, os.path.basename(sample_selection)))
    ...
    sample_extract = osof.otb_sample_extraction(stack_raster,sample_selection,field.lower(), outwd, float(version),split)
