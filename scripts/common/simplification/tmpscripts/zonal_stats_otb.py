...
def zonal_stats_otb(vecteur, r_validity, r_classif, r_classifier, field, outwd, version, out, ndept):
    ...
    poly_class_stats = osof.otb_polygon_class_stats(stack_raster, vecteur, field, outwd, float(version))
    timer.stop()
    ...
    shutil.copy(poly_class_stats, \
                out + "/dept_%s"%(ndept) + '/' + poly_class_stats_name)    
    
    timer.start()
    #sample selection (sample_selection.sqlite)
    sample_selection = osof.otb_sample_selection(stack_raster,vecteur, field, poly_class_stats, outwd, float(version))
    timer.stop()
    time_sample_selection = timer.interval

    sample_selection_name = os.path.basename(sample_selection)
    shutil.copy(sample_selection, \
                out + "/dept_%s"%(ndept) + '/' + sample_selection_name)    
    ...
    sample_extract = osof.otb_sample_extraction(stack_raster,sample_selection,field, outwd, float(version))
    timer.stop()
...
if __name__ == "__main__":
	...
	parser = argparse.ArgumentParser(description = "Regulararize a raster")
        ...
        shutil.copy(args.path+"/dept_%s"%(args.ndept)+"/sample_extract.sqlite", \
                    args.out+"/dept_%s"%(args.ndept)+"/sample_extract.sqlite")    
