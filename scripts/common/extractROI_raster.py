...
def getFieldType(layer,field):
	...
	try: return fieldDef[field]
	except : raise Exception('Field : "'+field+'" not in field\'s list : '+' , '.join(fieldList))
...
def generateRaster(raster_path,features_path,driver,field,valField,output,epsg):
	...
	if features is None: raise Exception("Could not open "+features_path)
