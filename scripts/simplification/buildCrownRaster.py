...
def manageBlocks(pathCrowns, tilenumber, blocksize, inpath, outpath, ram, logger=logger):
    ...
    for paths, dirs, files in os.walk(pathCrowns):
        for crown in files:
            if "_" + str(tilenumber) + ".tif" in crown:
                ...
                for y in intervalY:
                    for x in intervalX:
                        outputTif = os.path.join(inpath, "block%s.tif"%(nbblock))
                        roiapp = OtbAppBank.CreateExtractROIApplication({"in": os.path.join(inpath, crown),
                                                                         "ram": ram,
                                                                         "startx": x,
                                                                         "starty": y,
                                                                         "sizex": blocksize,
                        ...
                        x = labels * masknd
                        outRasterPath = os.path.join(inpath, "block%s_masked.tif"%(nbblock))
                ...
                fu.assembleTile_Merge(tomerge, int(ds.RasterXSize), out, ot="Byte"):
