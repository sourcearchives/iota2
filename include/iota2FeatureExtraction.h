...
template <typename T>
constexpr T normalized_index(const T refl, const T refrefl, 
                             const bool acorfeat=false, 
                             const T acorepsilon=0.05,
                             const T epsilon=10e-6)
{
  if(!acorfeat)
    {
                                   ...
                                   size_t date_counter)
    {
      ...
      if(!(m_RelativeReflectances && m_RemoveDuplicates && m_ReferenceIndex == refindex))
        ...
        outVec[copyOffset+m_NumberOfDates*featureOffset+date_counter] = 
          normalized_index(refl, refrefl, m_ACorFeat) * m_NormalizedIndexFactor;
        ++result;
        }
      return result;
    }
