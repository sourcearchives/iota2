  ...
  PixelType operator()(const PixelType& p)
  {
    ...
    while(inIt != inVec.cend())
      ...
      if(std::any_of(inIt, inIt+m_ComponentsPerDate,
        ...
        auto ndvi = std::fabs(nir-red)<10e-6?ValueType{0}:(nir-red)/(nir+red);
        auto ndwi = std::fabs(nir-red)<10e-6?ValueType{0}:(swir-nir)/(swir+nir);
	//auto ndvi = std::fabs(nir-red)<10e-6?ValueType{0}:std::fabs(nir+red)<10e-6?ValueType{0}(nir-red)/(nir+red);
        //auto ndwi = std::fabs(swir-nir)<10e-6?ValueType{0}:std::fabs(swir+nir)<10e-6?ValueType{0}(swir-nir)/(swir+nir);                     
