...
| sampleAugmentation            | parameter to set sample augmentation strategies(3)                     | not mandatory, defautl = {"activate" : False}                      | python dictionnary format                         | (3)                                         |
...
sampleAugmentation : {"target_models":["1", "2"],#could be "all"
                      ...
                      "strategy.smote.neighboatLeastrs"  : 5,
                      "samples.strategy" : "balance",#atLeast/balance/byClass 
...
atLeast : An option to generate samples so that all the classes have at least N samples.
...
samples.strategy.atLeast : number of the minimum required samples by class.
